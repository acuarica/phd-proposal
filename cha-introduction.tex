...
\cancel{that}
the native code
\cancel{will not compromise the entire system}.
...
% REVIEW: For the thesis (not here) What conclusions did they draw on JNI usage?
...
thus understand%
\cancel{ing} how \todo{those -> these} mechanisms are used.
These patterns can provide an insight \todo{on -> into} how the language is being used by developers in real-world applications.
...
\todo{Does the Unsafe API ... To what extent ... }
{\bf Does Unsafe impact common application code?}
We want to understand to what extent \todo{third-party: what do you mean?}
code actually uses Unsafe.

% REVIEW: Why underlined
\item\label{unsafe:rq2}
{\bf \underline{Why} \todo{How and when} are Unsafe features used?}
...
{\bf How recurrent are the \underline{reasons} \todo{patterns} for which casts are used?}
...
\marginpar{\todo{Why is this not asked for Unsafe? Can you ask the same 3 RQs for each?}}
