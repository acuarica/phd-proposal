...
% TODO:
% Static vs. Dynamic analysis in the literature review
...
% 2.1 Repositories - what is out there
% 2.2 Corpora - what to analyze
% 2.3 Tools - how to analyze (static/dynamic, BOA, QL, ...)
% 2.4 Questions -what questions to answer (especially where they get close to type systems)
...
\todo{the}
computer science literature about empirical studies of programs\todo{; -> grammar}
\underline{which involves several directions directly or indirectly} related to our topic.
\underline{Along} the last decades, researchers always ha\todo{s -> ve} been interested in understanding what kind of programs developers write.
The motivation behind these studies is quite broad and --- together with the evolution of computer science itself --- has
\todo{?}
shifted \todo{to} the needs of researchers.

For instance, \cancel{already}~\cite{knuthEmpiricalStudyFORTRAN1971} started to study \fortran{} programs.
...
\marginpar{\todo{Basic, move earlier}}
Alternatively, to measure the advantages between compilation and interpretation in \basic{}, \cite{hammondBASICEvaluationProcessing1977} \cancel{has} studied a representative dataset of programs.
Adding to Knuth's work, \cite{shenEmpiricalStudyFortran1990} \todo{made} an empirical study for parallelizing compilers.
...
\todo{At least add a statement here acknowledging all the MSR work.}
\marginpar{\todo{Nothing after 1990?}}

The organization of the rest of this chapter is as follows: 
How benchmarks and corpora relate to this kind of stud\todo{ies->y} is presented in \S\ref{sec:lr:benchmarks}.
...
\todo{made a study -> conducted a study}

\section{Benchmarks and Corpora}
\label{sec:lr:benchmarks}

\marginpar{OOPSLA'17 code duplicates in GitHub}
\marginpar{SPEC, etc}
Benchmarks are crucial to properly evaluate and measure product development.
This is key for both research and industry.
One popular benchmark suite for \java{} is \todo{the} DaCapo~\citep{blackburnDaCapoBenchmarksJava2006}.
...
\marginpar{\todo{Some discussion of the representativeness of the corpora}}
...
One important note is that they choose relevant project\todo{s} by popularity,
...
\marginpar{\todo{Avoid "note that", "notice that"}}
...
The work done by~\cite{nakshatriAnalysisExceptionHandling2016} categorized them in\todo{to} patterns.
\cancel{Whether}
...
\subsection*{\cancel{Functional} Programming \todo{Language} Features}
...
\section{Code Patterns Discovery \todo{Body of this section seems to be on tools, not on pattern discovery}}
...
However, this is only a meta-pattern detector, and not a pattern discovery.\todo{Why?}
\marginpar{\todo{What's a pattern?}}
...
\marginpar{\todo{Redundant}}
\marginpar{\todo{Why is Rascal relevant?}}
...
As mentioned above, \cite{dietrichContractsWildStudy2017a} \todo{made a study} about how programmers use contracts in \mavencentral{}.
For their analysis\footnote{\url{https://bitbucket.org/jensdietrich/contractstudy}},
they \cancel{have} use\todo{d} JavaParser\footnote{\url{http://javaparser.org/}}.
The main issue with JavaParser is the lack \underline{\todo{to do}} symbol resolution integrated with the project dependencies.
...
\emph{.QL} \cite{moorKeynoteAddressQL2007} \todo{Fix citation},
...
\marginpar{too general}
\marginpar{Should be statistical information?}
...
The same authors of \boa{} \todo{made a study} on
how new features
\todo{Which features? Any relevant for your work?}
...
\cite{lopesDeJaVuMapCode2017} \todo{made a study} to measure code duplication in \github{}.
They found out that much of the code there is actually duplicated.
This raises a flag when consider\todo{ing} which projects \todo{to} analyze when \cancel{doing} mining software repositories.
\marginpar{\todo{Related to Lopes}}
...
project age, activity, and \underline{of the last 12 months}\todo{?}, number of contributors, total code churn, and number of commits.
\marginpar{Move to 2.1}
...
and conducted a \todo{unscientific user} survey to study how \unsafe{} is used~\citep{psandoz14}.
...
\todo{We go -> Sounds wrong here. This wording fits in a paper, not a proposal.}
beyond Sandoz' work
by performing a comprehensive study of the \mavencentral{} software repository to analyze how and why \smu{} is being used.
\todo{This study} is summarized in Chapter \ref{cha:unsafe}.

\todo{Oracle provides -> This para should go first, no?} the \smu{} class for low-level programming, e.g, synchronization primitives, direct memory access methods, array manipulation and memory usage.
Although the \smu{} class is not officially documented,
\todo{there is literature based on it -> Who cares that there is literature. Motivate the contents of that literature by putting a better glue sentence here.}.

\marginpar{Relevant?}
\marginpar{These are users of Unsafe, not studies of it}
...
They used the \code{allocateInstance} method, because it eases the creation of instances even if the class has no default constructor.
...
\section{Casting -> \todo{Intro sentence needed}}
...
\todo{categorization according to semantic patterns -> Makes it sound like “semantic patterns” was a well-known concept. Don’t start with terms like this, start with description instead.}.

\cite{tsantalisJDeodorantIdentificationRemoval2008} present an Eclipse plug-in that identifies
\todo{type-checking bad smells -> Briefly connect to casts}.
...
\subsection*{Controlled Experiments on Subjects \todo{Shouldn't that 2.8?}}
...
For instance, \cite{solowayEmpiricalStudiesProgramming1984} tried to understand \cancel{the} how expert programmers face problem solving.
