...
% TODO: Third example, explain better
...
including the term \code{ClassCastException} on \github{} returns
\cancel{\circa{}}\todo{about}
$150K$
% and
% $72K$
results.
...
\todo{a few source code results -> Are these really the best motivating/illustrative examples?}.
This illustrates the sort of problem\cancel{s} developers have when applying casting conversions.
...
the \code{getProperty} method obtains a dynamic property.
If it \todo{founds -> find} a value, return\cancel{s} it.
\marginpar{grammar}
Otherwise, returns the default value (second argument).
\todo{In this case -> I don't see the cast here},
the property \code{"peer.p2p.pingInterval"} has type \code{int}.
...
\todo{Our proposal tries -> We propose to}
to answer the following question: \emph{\todo{How and when do} \todo{Why} developers need to escape the type system?}
The cast operator in \java{} provides the means to view a reference \todo{as -> at} a different type as it was \todo{defined->declared}.
Upcasts conversions are done automatically by the compiler.
Nevertheless, \todo{if->in} some situations a developer is forced to insert upcasts\todo{?}.
...
\item\label{enum:rq1}{\bf \todo{Is -> How frequently} casting used in common application code?}
...
we need to determine whether and how casting operations are actually used in real-world
\cancel{third-party}
\java{} applications.
...
\ql{} has the ability to query static types and \todo{perform} data-flow analysis.
...
\todo{It is common that a project exhibits more than one pattern -> Weird statement}.
...
\marginpar{Why only 24 project?}
\marginpar{Should say this is preliminary?}
...
\underline{This is why}\todo{no?} we want to exclude any primitive conversion in our study.
...
we want to include them \todo{for->in} our analysis.
...
Whenever we observe that those tags do not appear \underline{randomly}\todo{?},
...
To sum \cancel{it} up, our methodology iterates over the results until no patterns can be detected.
...
\begin{tikzpicture}[node distance = 1.5cm, auto]
    ...
    \node [decision, below of=tag, node distance=2.2cm] (pattern) {New Pattern?};
    \node [block, left of=tag, node distance=4cm] (update-tags) {Update Tags};
    \node [block, right of=pattern, node distance=4cm] (update-pattern) {Update Patterns};
    % \node [decision, below of=evaluate] (decide) {is best candidate better?};
    \node [block, below of=pattern, node distance=1.9cm] (stop) {Stop};
    % Draw edges
...
\marginpar{\todo{What about patterns we can't write queries for?}}
